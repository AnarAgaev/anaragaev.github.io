...
function sliders() {
	...
	var swiper4 = new Swiper('.main_news_slider .swiper-container', {
		loop: true,
		effect: 'fade',
		navigation: {
			nextEl: '.swiper-button-next',
	...
	var swiper5 = new Swiper('.our_instagram_slider .swiper-container', {
		slidesPerView: 6,
		spaceBetween: 10,
		scrollbar: {
			el: '.swiper-scrollbar',
	...
	new Swiper('.comparison__products .swiper', {
		loop: false,
		slidesPerView: 2,
		watchOverflow: true,
...
// Большой слайдер в шапке сайта --- START
let grandSlider

function initialGrandSliderVideo() {
	const videoSlides = Array.from(document
		.querySelectorAll('.grandslider__media:has(video)'))

	videoSlides.forEach(slide => {
		slide.addEventListener('click', grandSliderVideoClickHandler)
	});
}

function grandSliderVideoClickHandler() {
	const videos = Array.from(this.querySelectorAll('video'))

	videos.forEach(video => {
		if (getComputedStyle(video).display === 'block') {
			video.paused
				? playGrandSliderVideo(video, this)
				: pauseGrandSliderVideo(video, this)
		}
	});
}

function playGrandSliderVideo(video, container) {
	video.play()
	grandSlider.autoplay.pause()
	container.classList.add('playing')
}

function pauseGrandSliderVideo(video, container) {
	video.pause()
	grandSlider.autoplay.run()
	container.classList.remove('playing')
}

function pauseAllGrandSliderVideo() {
	const wrappers = Array.from(document
		.querySelectorAll('.grandslider__media:has(video)'))

	const videos = Array.from(document
		.querySelectorAll('.grandslider__media video'))

	videos.forEach(video => video.pause());
	wrappers.forEach(wrap => wrap.classList.remove('playing'))
}

function initGrandSlider() {
    grandSlider = new Swiper('.grandslider .swiper', {
		...
		on: {
			slideChange: pauseAllGrandSliderVideo
		}
    })
}

$(document).ready(() => {
	initialGrandSliderVideo()
	initGrandSlider()
})
...
function readMoreCatalog() {
  $('.catalog .catalog_element').slice(0, 4).addClass('el_active');
  $('.catalog .more_units').click(function () {
    $('.catalog .catalog_element').slice(4, 20).addClass('el_active');
  });
}
