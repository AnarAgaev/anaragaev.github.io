...
window.initInformers = (tooltip) => {
    ...
    const checkTooltipIsShowing = () => {
        if (tooltip.classList.contains('visible')) {
            tooltip.classList.remove('visible')
        }
    }

    const showTooltip = window.debounce(function () {
        if (!informer.classList.contains('visible')) {
            tooltip.classList.add('visible')

            showTooltipTimeoutId = setTimeout(checkTooltipIsShowing, 2000)
        }
    }, 300)
    ...
    btns.forEach(btn => {
        btn.addEventListener('mouseover', () => {
            ...
            const left = rect.right + window.pageXOffset
...
window.setInformerTooltip = (cf) => {
    if (!cf) return

    const tooltip = document.createElement('div')

    tooltip.classList.add('informer-tooltip')
    tooltip.id = 'informerTooltip'
    tooltip.role = 'tooltip'
    tooltip.innerText = 'Кликни, чтобы узнать подробнее'
...
window.updateInformers = () => {
    window.setInformerTooltip(window.initInformers)
}
